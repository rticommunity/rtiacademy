...
def test():
    "Join, send some messages to the group, then leave."

    import time

    # Test user data
    myuser = ChatUser(username="testuser", group="services", firstName="TestName")
    
    # Handlers simply print samples to stdout
    myhandlers = Handlers()
    def print_users_joined(*args, **kwargs):
        print("User joined:", args, kwargs)
    def print_users_dropped(*args, **kwargs):
        print("User dropped:", args, kwargs)
    def print_message(*args, **kwargs):
        print("Message received:", args, kwargs)
    myhandlers.users_joined = print_users_joined
    myhandlers.users_dropped = print_users_dropped
    myhandlers.message_received = print_message
    
    # Create app (auto joins)
    ddsapp = DDSApp(myuser, myhandlers)

    # Send a few messages to the group
    for i in range(1, 11):
        ddsapp.message_send(myuser.group, f"Hello {i}  ... waiting 1s")
        time.sleep(1)
    ddsapp.message_send(myuser.group, "Goodbye")
    time.sleep(1)
